...
|---|---|---|---|
| `3px` | 4 | `--space-2` (2px) | `.mode-toggle` padding, `.phase-badge`/`.lb-phase-badge` padding, `.exercise-detail` margin-top |
| `9px` | 4 | `--space-10` (10px) | `.plan-table th/td` padding, `.btn-announce-read`/`.btn-fb-status` padding, `.fb-type-btn` padding |
| `13px` | 3 | `--space-14` (14px) | `.auth-input` padding, `.nav-item` padding, `.post-session-bar` padding |
| `1px` | 1 | **left alone** | `.checklist-item input[type=checkbox]` `margin-top: 1px` — a sub-pixel optical nudge to align the checkbox with adjacent text, not layout rhythm; rounding it would visibly misalign it |
